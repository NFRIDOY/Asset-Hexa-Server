...
async function run() {
	try {
		...
		app.post("/transections", async (req, res) => {
			try {
				...
				if (typeTransec === "INCOME") {
					...
					// find the account
					const accountfindOne = await accountsCollection.findOne(
						queryAccount
					);
					...
					const resultTransec =
						await transectionsCollection.insertOne(newTransections);
					...
					// find the account
					const AccountFrom = await accountsCollection.findOne(
						fiterFrom
					);
					...
					AmountOnAccountForm =
						AmountOnAccountForm - newTransections?.amount;
					AmountOnAccountTo =
						AmountOnAccountTo + newTransections?.amount;
		...
		app.get("/totalInExp", async (req, res) => {
			...
			const allTrasIncome = cursorIncome?.map((tr) =>
				parseFloat(tr?.amount)
			);
			const allTrasExpense = cursorExpense?.map((tr) =>
				parseFloat(tr?.amount)
			);
		...
		app.get("/chartData/:email", async (req, res) => {
			...
			try {
				...
				const singlePersonData = await accountsCollection
					.find(query)
					.toArray();
		...
		async function getTotalBalance(email) {
			...
			const result = await accountsCollection
				.aggregate(pipeline)
				.toArray();
		...
		app.get("/catPi", async (req, res) => {
			try {
				...
				const cursor = await transectionsCollection
					.find(query)
					.toArray();
		...
		app.put("/businessInvest/:id", async (req, res) => {
			try {
				...
				const totalInvestment =
					thisBusiness?.totalInvestment + newInvestment;
				// console.log(id);
		...
		app.get("/adminState", async (req, res) => {
			...
			const businessCount =
				await businessesCollection.estimatedDocumentCount();
